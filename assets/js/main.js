/* ===== Journey Inside the Atom (Chapter 8) — shared behaviours ===== */
(function(){
  "use strict";
  ...
  /* ---------- nav menu: desktop dropdown + mobile Menu button ---------- */
  function syncDrop(d){
    var b=d.querySelector('button[aria-expanded]');
    if(b){ b.setAttribute('aria-expanded', d.classList.contains('open') ? 'true' : 'false'); }
  }
  function closeAll(){
    $$('.drop').forEach(function(d){ d.classList.remove('open'); syncDrop(d); });
    var mb=$('.menu-btn'); if(mb){ mb.setAttribute('aria-expanded','false'); }
  }
  function openDrop(d, force){
    if(!d) return;
    var on = (force===undefined) ? !d.classList.contains('open') : !!force;
    d.classList.toggle('open', on);
    syncDrop(d);
    var mb=$('.menu-btn'); if(mb){ mb.setAttribute('aria-expanded', on ? 'true' : 'false'); }
  }
  /* keep the mobile panel right under the sticky header */
  function placePanel(){
    var n=$('.nav');
  ...
  document.addEventListener('click', function(e){
    ...
    if(t.closest('.menu-btn')){ e.preventDefault(); openDrop($('.drop')); return; }
    if(t.closest('.drop-panel a')){ closeAll(); return; }
    var dd=t.closest('.drop');
    if(dd){ e.preventDefault(); openDrop(dd); return; }
    closeAll();
  });
  document.addEventListener('keydown', function(e){ if(e.key==='Escape' || e.keyCode===27){ closeAll(); } });
  window.addEventListener('resize', function(){ placePanel(); if(window.innerWidth>980){ closeAll(); } });
  window.addEventListener('orientationchange', placePanel);
  if(document.readyState!=='loading'){ placePanel(); } else { document.addEventListener('DOMContentLoaded', placePanel); }
  ...
  try {
    ...
    if (vsel) {
      var saved = null;
      try { saved = localStorage.getItem('voice'); } catch (e) {}
      if (saved) { vsel.value = saved; }
      vsel.addEventListener('change', function () {
        try { localStorage.setItem('voice', vsel.value); } catch (e) {}
        var t = vsel.options[vsel.selectedIndex].textContent;
        $$('.aud-row .meta').forEach(function (m) {
          if (m.getAttribute('data-vnote') === '1') { m.textContent = 'Narration voice: ' + t; }
        });
      });
    }
  } catch (e) {}
